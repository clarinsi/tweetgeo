...
spatialSignal<-function(df,attrNr){
  df<-df[,c(lonidx,latidx,attrNr)]
  names(df)<-c("y","x","attr")
  ...
  for(i in levels(df@data$attr)){
    ...
    output<-rbind(output,data.frame(feature=attrNr,level=i,distLevel=df.sub.dist,distFeature=df.dist,distRel=df.sub.dist/df.dist,nValues=length(df.sub)))
  }
  
  return(output)
}

mapTweets<-function(df,attrNr){
  df<-df[,c(latidx,lonidx,attrNr,textidx)]
